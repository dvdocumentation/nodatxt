...
.. attention::  Крайне важно помнить – если вы используете контейнеры, у элменто обязательно надо задавать ширину и высоту. У самих контейнеров как правило тоже. И также возможно Parameters у «строки»
...
Используются для выбора конкретного значения датасета на форме – элемента справочника, документа. Удобство заключается в том, что выбирается ссылка на объект по которой можно получить объект целиком или представление объекта. 
...
Методы узла (узел на устройстве)
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
...
Типы объектов(ключ type) бывают
 следующие:
...
*MediaGallery* – открывает камеру для фотографирования. По ключу = id элемента хранится массив путей к файлам, который отображается в галерее. С ним по умолчанию автоматически взаимодействуют PhotoButton и GalleryButton но также этот массив можно редактировать из обработчика (например поместить туда фото). 
...
convertImageFilesToBase64Array,

saveBase64ToFile,

convertBase64ArrayToFilePaths
